...
class experiment1():
    def __init__(self, session_type):
        ...
        self.PR_tally = 0
        self.blocks = [{'type':'cont1x', 'st':0}]

        # experiment parameters
        self.PR_reward = 3
        self.FR_reward = 1
        self.FR_cost = 6
        self.max_block_nums = 7
    ...
    def pull(self, action):
        ...
        if action == 0:
            ...
            raise ValueError
        if self.consecutive_FR >= self.max_consecutive_FR or self.current_trial - block_start >= 99:
            self.reset()
    ...
    def reset(self):
        ...
        if len(self.blocks) < self.max_block_nums:
            session_type_map = {0: ['cont1x'],
                    1: ['cont1x', 'cont2x'],
                    2: ['cont1x', 'disc1x'],
                    3: ['cont1x', 'cont2x', 'disc1x', 'disc2x']}
            block_type_choices = session_type_map[self.session_type]
            self.blocks.append({'type': random.choice(block_type_choices), 'st': self.current_trial + 1})
            self.PR_tally = 0
            self.consecutive_FR = 0
        else:
